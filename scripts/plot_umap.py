...
from cluster_fibers import (
    load_cache, set_preset, resolve_feature, feature_observed_mask,
...
def main():
    ...
    parser.add_argument("--feature-title", default=None,)
    ...
    if args.plot_feature:
        ...
        Xfull = Xfull[keep]
        Xfull_pre = Xfull  # pre-imputation (group_median_impute returns a copy)
        Xfull, _ = group_median_impute(Xfull, groups)
        vals = resolve_feature(args.plot_feature, Xfull, full_cols)
        obs = feature_observed_mask(args.plot_feature, Xfull_pre, full_cols)
        print(f"Feature {args.plot_feature}: {int(obs.sum()):,} mapped / {int((~obs).sum()):,} unmapped (white)")
        vmin = args.vmin if args.vmin is not None else float(vals[obs].min())
        vmax = args.vmax if args.vmax is not None else float(vals[obs].max())

        fig4, axes4 = plt.subplots(1, 4, figsize=(16, 4), sharex=True, sharey=True)
        for ax, g in zip(axes4, GROUPS):
            m = groups == g
            # unmapped fibers (not matched on the feature's slide) -> white
            ax.scatter(X_umap[m & ~obs, 0], X_umap[m & ~obs, 1],
                       facecolors="white", edgecolors="none", s=6, alpha=0.25)
            ax.scatter(X_umap[m & obs, 0], X_umap[m & obs, 1], c=vals[m & obs],
                       cmap="viridis", vmin=vmin, vmax=vmax, s=6, alpha=0.25,
                       edgecolors="none")
            ax.text(0.1, 0.9, g, transform=ax.transAxes, fontsize=12, fontweight="bold",
                    ha="left", va="top", color="black")
            ax.set_xticks([])
            ax.set_yticks([])
            for spine in ax.spines.values():
                spine.set_visible(False)
        # feature name 
        if args.feature_title is not None:
            axes4[0].set_ylabel(args.feature_title, fontsize=14, fontweight="bold")
        else:
            axes4[0].set_ylabel(args.plot_feature, fontsize=14, fontweight="bold")
        # colorbar
